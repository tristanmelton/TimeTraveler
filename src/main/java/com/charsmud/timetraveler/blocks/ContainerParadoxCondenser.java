...
import com.charsmud.timetraveler.blocks.slots.SlotParadoxCondenserFuel;
import com.charsmud.timetraveler.blocks.slots.SlotParadoxCondenserOutput;
...
public class ContainerParadoxCondenser extends Container 
{
	private final TileEntityParadoxCondenser tile_entity;
	private int cookTime, totalCookTime, burnTime, currentBurnTime; 
	
	public ContainerParadoxCondenser(InventoryPlayer player, TileEntityParadoxCondenser tile_entity)
	{
		this.tile_entity = tile_entity;
		this.addSlotToContainer(new Slot(tile_entity, 0, 37, 14));
		this.addSlotToContainer(new SlotParadoxCondenserFuel(tile_entity, 1, 37, 52));
		this.addSlotToContainer(new SlotParadoxCondenserOutput(player.player, tile_entity, 2, 92, 33));
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
	{
		...
		if(slot != null && slot.getHasStack())
		{
			...
			{
				if(!ParadoxCondenserRecipes.getInstance().getCondensingResult(stack1).isEmpty())
				{
					if(!this.mergeItemStack(stack1, 0, 1, false))
						return ItemStack.EMPTY;
					else if(TileEntityParadoxCondenser.isItemFuel(stack1))
					{
